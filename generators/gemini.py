...
def build_base_prompt(
    ...
    hook_style = random.choice(POST_HOOK_STYLES)
    ending_style = random.choice(POST_ENDING_STYLES)
    banned_str = "; ".join(f"«{phrase}»" for phrase in BANNED_AI_PHRASES)

    return f"""{EDITORIAL_TEAM_BRIEF}

Ти — {persona['name']}, {persona['role']}.
Канал "ФінПро для дітей" — Telegram-канал для підлітків України 12–20 років.
Тематика каналу: гроші й технології плюс цікавий світ — наука, космос,
природа, географія, історія. Головне — щоб було цікаво читати і по-справжньому
корисно, а не «пізнавальна довідка».
...
- ЗАБОРОНЕНІ фрази (жодної з них у тексті): {banned_str}
- Не пояснюй читачеві окремим блоком «чим це корисно» — користь має бути видна із самої історії, деталі чи цифри
- Не звертайся до читача формулою «для тебе це…»; говори про ситуацію, а не про читача
...
Про ФОРМАТ нижче:
- Це орієнтир структури, а не шаблон для дослівного копіювання
- Підписи-ярлики («Проблема:», «Як це працює:») можна перефразувати живою мовою або прибрати, якщо без них читається природніше
- Рядок з 💬 — це місце для фіналу; використай обраний тип фіналу, питання не обов'язкове
